...
var warning = function (content) {
    if (!content) return
    process.stdout.write(require('ansicolors').brightGreen('\r\n' + content))
}
...
var merge = function (source, addon, ignore) {
    forEach(addon, function (value, key) {
        if (!!source[key] && value !== source[key]) {
            warning(['Dunplicate i18n key of "', key, '" with different values, merged from "', source[key], '" to "', value, '".'].join(''))
        }
...
exports.default = function (content) {
    ...
    if (prod) {
        ...
        forEach(content, function (langData, langKey) {
            ...
            // merge and write
            try {
                orgin = merge(orgin, langData, option.ignoreEmpty)
        ...
        if (option.extend) {
            ...
            forEach(extend, function (extendLangData, extendLangKey) {
                ...
                // merge and write
                try {
                    orgin = merge(orgin, extendLangData, option.ignoreEmpty)
                    fs.writeFileSync(targetJson, JSON.stringify(orgin), {encoding: option.character || 'utf-8'})
                } catch (err) {
                    mergeError.push(err)
                }
