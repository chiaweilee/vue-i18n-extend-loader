...
var safeCheck = function (content) {
    if (content.indexOf('!') > -1) {
        warning('\r\nDo not use exclamation mark in lang package(both key and value), it will cause loader fail.')
    }
}
        ...
        forEach(content, function (langData, langKey) {
            ...
            try {
                ...
                if (typeof mirror === 'object' && !!mirror.target && mirror.source === langKey) {
                    ...
                    const mirrorData = read(mirrorJson)
                    Object.keys(orgin).forEach(key => {
                        if (!mirrorData[key]) {
                            mirrorData[key] = orgin[key]
                        }
                    })
                    fs.writeFileSync(mirrorJson, JSON.stringify(mirrorData), {encoding: option.character || 'utf-8'})
                }
            } catch (err) {
                mergeError.push(err)
            }
